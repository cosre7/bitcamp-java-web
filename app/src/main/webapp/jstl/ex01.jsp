...
- JSTL(JSP Standard Tag Library)? 
  - JSP 확장 태그이다.
  - 기본으로 제공하지 않는다. -인터페이스는 있지만 구현 클래스가 없다.
...
- JSP 페이지에서 JSTL 라이브러리의 모듈 사용하기
  - JSTL 모듈의 네임스페이스를 가져온다. -> 정확하게 작성해야 한다.
    prefix : 별명 -> 마음대로 해도 상관없지만 Core 모듈을 쓸 땐 c, sql 관련 모듈을 쓸 땐 sql ...
    보통 라이브러리 모듈에 따른 별명을 사용한다.
...
<%--템플릿 데이터 
    밑의 out.println~ 과 같은 형태이지만
    <h2>~ 와 같이 쓰자!
    
    값을 출력하는 다양한 방법! --%>
<h2>오호라!!!</h2> <%-- = out.write("오호라!!!"); --%>

<%
out.println("<h2>오호라!!!</h2>");
%>

<%="<h2>오호라!!!</h2>"%> <%--out.println("<h2>오호라!!!</h2>") --%> 

${"<h2>오호라!!!</h2>"}

<c:out value="<h2>오호라!!!<h2>"/> <%-- <>를 일반 텍스트 취급한다. --%>
