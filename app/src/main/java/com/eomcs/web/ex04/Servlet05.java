...
// 방법 2) 애노테이션으로 설정하기
//
@MultipartConfig(maxFileSize = 1024 * 1024 * 10) // 10MB // @MultipartConfig 가 없으면 멀티파트가 아니다.
@WebServlet("/ex04/s5")
public class Servlet05 extends GenericServlet {
  ...
  public void service(ServletRequest req, ServletResponse res)
    ...
    if (photoPart.getSize() > 0) {
      // 파일을 선택해서 업로드 했다면(사이즈가 0 이상이라면),
      // 파일을 보내지 않았으면 사이즈는 0일 것이다.
