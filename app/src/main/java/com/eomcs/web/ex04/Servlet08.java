...
public class Servlet08 extends GenericServlet {
  ...
  public void service(ServletRequest req, ServletResponse res)
    ...
    // 실무에서는 ~~~_20x20.jpg 형태의 이름으로 쓴다.

    Builder<File> thumbnailBuilder = Thumbnails.of(this.uploadDir + "/" + filename); // 경로명
    thumbnailBuilder.size(20, 20);
    thumbnailBuilder.outputFormat("jpg");
    thumbnailBuilder.toFiles(Rename.PREFIX_DOT_THUMBNAIL); // 썸네일 이미지의 파일 이름에 Tumbnail이라고 붙이기 -> 실무에서 잘 안씀

    Builder<File> thumbnailBuilder2 = Thumbnails.of(this.uploadDir + "/" + filename);
    thumbnailBuilder2.size(20, 20);
    thumbnailBuilder2.crop(Positions.CENTER); // 옆이 길면 잘라서 줄인다.
    ...
    Thumbnails.of(this.uploadDir + "/" + filename) // thumbnailBuilder 라는 변수명에 담지 않고 바로 사용해도 무관하다.
    .size(80, 80)
    .outputFormat("jpg")
